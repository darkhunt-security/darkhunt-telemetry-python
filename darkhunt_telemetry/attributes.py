"""OTLP attribute keys the SDK emits.

Mirrors ``src/attributes.ts`` in the TypeScript SDK exactly — the trace-hub
backend reads these exact keys, so they are the wire contract and must not
drift between the two SDKs.
"""
...
class ATTR:
    ...
    # backend iterates attributes by this prefix and strips it back off, so each
    # entry must live in its own attribute — a single JSON-blob attribute is not
    # iterable that way and gets dropped on ingest.
...
class GEN_AI:
    ...
    # OTel GenAI semantic convention for structured chat input/output.
    # JSON-encoded array of {role, content}.
    ...
    # ``gen_ai.tool.name`` to the observation's tool name, which the dashboard
    # uses as the span title for ``tool`` observations.
